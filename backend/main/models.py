...
# In backend/main/models.py

class CohortCourse(models.Model):
    semester_number = models.IntegerField()
    course_name = models.CharField(max_length=200)
    is_lab = models.BooleanField(default=False)  # Cohort courses are typically always False
    duration = models.IntegerField(default=75)  # Always 75 minutes for theory
    capacity = models.IntegerField(default=100)  # Higher capacity
    
    class Meta:
        verbose_name = "Cohort Course"
        verbose_name_plural = "Cohort Courses"
    
    def __str__(self):
        return f"S{self.semester_number}: {self.course_name}"
